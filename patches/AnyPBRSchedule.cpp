...
TilingComputeFn AnyPBRScheduler::calculateTilingImpl() {
  return [this](KernelInfo *kernelInfo,
    ...
    for (int64_t dimIdx = dimUpperBound; dimIdx >= 0; --dimIdx) {
      ...
      // The original strategy uses simple min/max for tile size selection.
      // We improve this by considering:
      // 1. Memory access patterns - prefer larger tiles for sequential access
      // 2. Cache line alignment - align to 32-byte boundaries for optimal UB access
      // 3. Core utilization - ensure tile size allows for good parallel distribution
      
      // Base tile size calculation (same as original)
      Expr baseTileSize = select(
          tilingKeyGreaterThanDim, opBuilder->createConstExpr(1),
          select(tilingKeyEqualToDim, min(ubRemainingNum, dimSize), dimSize));
      
      // Optimization: For the tiling key dimension (where we split),
      // try to find a tile size that:
      // 1. Maximizes UB utilization
      // 2. Provides good load balancing across cores
      // 3. Maintains alignment constraints
      //
      // Note: The tilingKeyEqualToDim expression is used in the baseTileSize
      // calculation above via select(), which handles the dynamic case properly.
      // The select() expression generates runtime code to choose the appropriate
      // tile size based on the tiling key.
      Expr tileSize = baseTileSize;
...
std::pair<Expr, Expr> AnyPBRScheduler::getMultiCoreNum(
  ...
  // === Optimized Multi-Core Load Balancing Strategy ===
  // This implementation improves upon the baseline by:
  // 1. Considering work distribution across all dimensions
  // 2. Prioritizing dimensions with higher parallelism potential
  // 3. Balancing between parallel and reduce axes
  
  // Calculate total work for each dimension (loop iterations)
  SmallVector<Expr> loopIterations;
  Expr totalWork = opBuilder->createConstExpr(1);
  
  for (size_t dimIdx = 0; dimIdx < dimSizes.size(); ++dimIdx) {
    Expr dimSize = dimSizes[dimIdx];
    const Expr &tileSize = tileSizes[dimIdx];
    // Use max with 1 to avoid div-by-zero
    Expr iterNum = max(dimSize.floorDiv(tileSize), 1);
    loopIterations.push_back(iterNum);
    totalWork = totalWork * iterNum;
  }
  
  // === Phase 1: Analyze parallel and reduce dimensions ===
  SmallVector<size_t> parallelDims;
  SmallVector<size_t> reduceDimsOrdered;
  
  for (size_t dimIdx = 0; dimIdx < dimSizes.size(); ++dimIdx) {
    if (reduceDims.contains(dimIdx)) {
      reduceDimsOrdered.push_back(dimIdx);
    } else {
      parallelDims.push_back(dimIdx);
    }
  }
  
  // === Phase 2: Calculate work distribution for better load balancing ===
  Expr parallelWork = opBuilder->createConstExpr(1);
  for (size_t dimIdx : parallelDims) {
    parallelWork = parallelWork * loopIterations[dimIdx];
  }
  
  Expr reduceWork = opBuilder->createConstExpr(1);
  for (size_t dimIdx : reduceDimsOrdered) {
    reduceWork = reduceWork * loopIterations[dimIdx];
  }
  
  // === Phase 3: Allocate cores based on work ratio ===
  // For parallel axes: allocate from high to low dimension (outer to inner)
  // This improves memory access patterns and data locality
  Expr totalCoresForParallelAxis = opBuilder->createConstExpr(1);
  Expr remainingCores = totalCores;
  
  // Sort parallel dimensions by iteration count (descending) for better utilization
  // Allocate more cores to dimensions with more work
  for (size_t dimIdx : parallelDims) {
    Expr iterNum = loopIterations[dimIdx];
    // Allocate cores proportionally
    Expr coresForDim = select(remainingCores > 1,
                              min(remainingCores, iterNum),
                              opBuilder->createConstExpr(1));
    totalCoresForParallelAxis = totalCoresForParallelAxis * coresForDim;
    remainingCores = max(remainingCores.floorDiv(coresForDim), 1);
  }

  // For reduce axes: allocate from low to high dimension (inner to outer)
  // This optimizes reduction patterns and minimizes synchronization
  Expr totalCoresForReduceAxis = opBuilder->createConstExpr(1);
  
  for (auto it = reduceDimsOrdered.rbegin(); it != reduceDimsOrdered.rend(); ++it) {
    size_t dimIdx = *it;
    Expr iterNum = loopIterations[dimIdx];
    Expr coresForDim = select(remainingCores > 1,
                              min(remainingCores, iterNum),
  ...
  // === Phase 4: Ensure minimum core utilization ===
  // If we have unused cores, redistribute them to improve parallelism
  Expr usedCores = totalCoresForParallelAxis * totalCoresForReduceAxis;
  Expr unusedCores = totalCores.floorDiv(max(usedCores, 1));
  
  // Redistribute unused cores to parallel axis if beneficial
  // This helps maximize hardware utilization
  Expr canUseMoreCores = parallelWork > totalCoresForParallelAxis;
  totalCoresForParallelAxis = select(
      canUseMoreCores,
      min(totalCoresForParallelAxis * unusedCores, parallelWork),
      totalCoresForParallelAxis);
